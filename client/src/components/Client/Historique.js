...
const Cart = () => {
  // Read the live cart from redux so add / remove / delete update instantly
  const mealData = useSelector((state) => state.cartreducer.carts);
  const [price, setPrice] = useState(0);

  const dispatch = useDispatch();

  // Keep localStorage in sync so the order form (Form.js) still works
  useEffect(() => {
    reactLocalStorage.set("mealData", JSON.stringify(mealData));
  }, [mealData]);
                  ...
                  <div className="flex-wrap md:flex md:justify-between w-full my-4">
                    ...
                    <div className="flex">
                      ...
                      <span
                        className="bg-amber-500 btn w-8 h-8 text-center font-bold rounded-full"
                        style={{ cursor: "pointer" }}
                        onClick={meal.quantity <= 1 ? () => Delete(meal._id) : () => DeleteOne(meal._id)}
                      >
                        -
                      </span>
                    </div>
