...
export default function Form() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/client/cash`, data)
      const paymentId = res.data._id
      const d = { client, dt: paymentId, produit, quantite }
      await axios.post(`${process.env.REACT_APP_API_URL}/manager/add-command`, d)
      // Order placed -> go back to the meals page
      navigate('/Client')
    } catch (err) {
      console.log(err)
    }
