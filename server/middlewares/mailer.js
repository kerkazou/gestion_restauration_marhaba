...
function main(method, user) {
  ...
  // Front-end base URL (falls back to localhost if not set in .env)
  ...
  if (method == 'addLivreur') {
    ...
    html = `<div>
            ...
            <div>
              ...
              <a href="${clientUrl}/login" style="background-color: #f59e0b; border: none;color: white;padding: 15px 32px; text-align: center; text-decoration: none;display: inline-block;">
                Log here
              </a>
            </div>
            `
  ...
  // Send the email but never let a mail failure crash the server.
  transporter
    .sendMail(info)
    .then(() => console.log("Message sent"))
    .catch((err) => console.log("Email not sent (check EMAIL/PASSWORD in .env):", err.message));
}
